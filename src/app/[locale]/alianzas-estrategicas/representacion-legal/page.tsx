...
const i18nNamespaces = ['representación-legal', 'common'];

const phrasesToHighlight = [
  'podemos servir como representantes legales'
];

export default async function Home({ params: { locale } }: { params: { locale: string } }) {
  const { t, resources } = await initTranslations(locale, ['representación-legal', 'navbar', 'mailing-list', 'footer']);
          ...
          <div className={styles.texts}>
            ...
            <div className={styles.linkContainer}>
              <div className={`${styles.linkWrapper} ${styles.leftAlign}`}>
                <Link href="/alianzas-estrategicas/asesoria-usa" className={styles.serviceLink}>{t('servicio-anterior')}
                </Link>
                <p className={styles.linkTitle}>{t('navbar:asesoria-usa')}</p>
              </div>
              <div className={`${styles.linkWrapper} ${styles.rightAlign}`}>
                ...
                <p className={styles.linkTitle2}>{t('navbar:cfo-on-demand')}</p>
              </div>
            </div>
          </div>
        </div>
