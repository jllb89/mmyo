...
const phrasesToHighlight = [
  'que se desee adquirir o invertir,',
  'cumplimiento de sus obligaciones fiscales e identificando los riesgos'
];

export default async function Home({ params: { locale } }: { params: { locale: string } }) {
  const { t, resources } = await initTranslations(locale, ['due-diligence-fiscal', 'navbar', 'mailing-list', 'footer']);
          ...
          <div className={styles.texts}>
            ...
            <div className={styles.linkContainer}>
              <div className={`${styles.linkWrapper} ${styles.leftAlign}`}>
                <Link href="/fiscal/promociones-fiscales" className={styles.serviceLink}>{t('servicio-anterior')}
                </Link>
                <p className={styles.linkTitle}>{t('navbar:dictamenes-fiscales')}</p>
              </div>
              <div className={`${styles.linkWrapper} ${styles.rightAlign}`}>
                <Link href="/fiscal/revision-fiscal" className={styles.serviceLink}>{t('siguiente-servicio')}
                </Link>
                <p className={styles.linkTitle2}>{t('navbar:revision-fiscal')}</p>
              </div>
            </div>
          </div>
        </div>
